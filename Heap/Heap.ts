export class Heap<T> {
  ...
  topDown():void{
    ...
    while(fn(target,leftChildNode) || fn(target,rightChildNode)){
      let [tempIdx,temp] = getTempChildData(rightChildIdx,leftChildIdx,this.tree);
      this.tree[idx] = temp;
...
const getTopDownNode:getTopDownNodeType = (idx,tree) => [tree[idx*2 + 1], tree[idx * 2 + 2]]

type getBottomUpType = <T>(idx:number, tree:T[]) => [number,T]
const getBottomUpData:getBottomUpType = (idx,tree) => [Math.floor((idx + 1) / 2 ) - 1,tree[Math.floor((idx + 1) / 2 ) - 1]]

type getTempChildDataType = <T>(right:number,left:number,tree:T[]) => [number,T];
const getTempChildData:getTempChildDataType = (rightChildIdx,leftChildIdx,tree) => {
